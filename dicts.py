ru_dict = {'контакты': 'Наши контакты: +7-800-350-55-26',
           'сведения об организации': 'Наша группа в ВК: https://vk.com/public212550123\nИнститут ПКиПП реализует '
                                      'обучение по программам ДПО, в том числе с применением дистанционных '
                                      'технологий в соответствии с требованиями к дистанционным программам. '
                                      '\nДля обеспечения полноты и '
                                      'качества освоения образовательной программы Институт ПКиПП имеет необходимую '
                                      'материально-техническую базу и дополнительный практико-ориентированный материал.'
           }
ru_book = ['Творческая мастерская ГОРНИЦА', 'Спорт', 'ОГЭ/ЕГЭ', 'Иностранные языки', 'Стажировки',
           'Повышение квалификации и профессиональная переподготовка', 'Шахматы', 'Коворкинг', 'Аренда конференц-зала',
           'Подготовка к школе', 'Поиск товаров из Китая', 'Перевод документов, заверение у нотариуса',
           'Психологическое консультирование, психокоррекция', 'ОВЗ', 'Кинорежиссура', 'Детский лагерь', '⬅Назад',
           'Ушу', 'Клуб самообороны "Святославъ"', 'Студия гимнастики', 'Взрослые', 'Дети', 'Репетиции',
           'Все предметы', 'Китайский', 'Остальные', 'Китай', 'Индия', 'Вьетнам', 'Другие', 'Контакты',
           'Сведения об Организации', 'Запись', '🏴󠁧󠁢󠁥󠁮󠁧󠁿EN']
ru_answers = [
    'Здравствуйте, уважаемый клиент!\nЯ бот Института ПКиПП.\nПожалуйста, выберите из меню то, что Вас интересует.',
    'Выберите из меню пункт для записи', 'Выберите возрастную категорию', 'Выберите спортивную секцию',
    'Выберите необходимую категорию', 'Выберите необходимый язык', 'Выберите страну', 'Укажите Ваше имя и фамилию',
    'Укажите свой номер телефона', 'Я Вас не понял, для подсказки напишите команду /help или /start',
    'Отлично, я записал Вас.']
en_dict = {'contacts': 'Our contacts: +7-800-350-55-26',
           'company information': 'Our VK group: https://vk.com/public212550123\nInstitute of PKIPP implements '
                                  'learning under programs of additional professional '
                                  'education, including using remote technologies in accordance with the requirements '
                                  'for remote programs\nTo ensure the completeness and quality of '
                                  'mastering the educational program The Institute of PKIPP has the necessary material '
                                  'and technical base and additional practice-oriented material.'
           }
en_book = ['Creative workshop GORNITSA', 'Sport', 'OGE/EGE', 'Foreign languages', 'Internships',
           'Professional development and professional retraining', 'Chess', 'Coworking', 'Rent of a conference hall',
           'Preparing for school', 'Search for products from China', 'Translation of documents, notarization',
           'Psychological counseling, psychocorrection', 'OVZ', 'Filmmaking', "Children's camp", '⬅Back', 'Wushu',
           'Self - Defense Club "Svyatoslav"', 'Gymnastics Studio', 'Adults', 'Kids', 'Rehearsals', 'All subjects',
           'Chinese', 'Other', 'China', 'India', 'Vietnam', 'Other', 'Contacts', 'Company information', 'Registration',
           '🇷🇺RU']
en_answers = [
    "Hello, dear customer!\nI'm a bot of the Institute PKIPP.\nPlease select from the menu what you are interested in.",
    'Choose the item from the menu to make an appointment', 'Choose an age category', 'Choose a sports section',
    'Choose the desired category', 'Choose the desired language', 'Select a country', 'Enter your first and last name',
    'Enter your phone number', "I didn't understand you, for a hint, write the command /help or /start",
    'Great, I signed you up.']
